...
PTR(Expr)parse_expr(std::istream &in) { //incorporate parse_multicand
    PTR(Expr)e;
    e = parse_comparg(in);
      skip_whitespace(in);
      int c = in.peek();
...
std::string parse_keyword(std::istream & in) {
    PTR(Expr) kw = parse_variable(in);
      PTR(VarExpr) other_e = CAST(VarExpr)(kw);
      std::string var = other_e->to_string();
      if (var == "let" || var == "in") {
          return var;
      }
...
PTR(Expr) parse_let (std::istream& in) {
    PTR(Expr) lhs;
     ...
     std::string kw, var;
     skip_whitespace(in);
     lhs = parse_variable(in);
     PTR(VarExpr) other_lhs = CAST(VarExpr)(lhs);
     var = other_lhs->to_string();
     skip_whitespace(in);
     consume(in, '=');
     rhs = parse_expr(in);
     skip_whitespace(in);
